...
/*
        ...
        var postData = {
            blockly: postJsonCollection["blockly"],
        }
*/
...
function restore() {//復元
    if (outputJson != null) {
        const inputJson = JSON.parse(outputJson);
        //BlocklyWorkspaceの復元
        ...
        const backgroundNode = document.getElementById("BGinformation");
        while (backgroundNode.firstChild) {
            if (backgroundNode.firstChild.id != "bg_div") {
                console.log(backgroundNode.firstChild);
                backgroundNode.removeChild(backgroundNode.firstChild);
            }
        }
        for (let key in inputJson["object_data"]["objects"]) {
            set_box_spr(inputJson["object_data"]["objects"][key].id, sekiinputJsonne2["object_data"]["objects"][key].src, inputJson["object_data"]["objects"][key].width, inputJson["object_data"]["objects"][key].height);
        }
    }
}
