...
public class Sign_up_Controller {
	...
	public String displayAdd2(Model Model,@Valid @ModelAttribute("form") SignupForm form, BindingResult result,RedirectAttributes redirectAttributes, HttpServletRequest request) {
		
		//emai重複用変数
		Model.addAttribute("emailC",false);

		if(result.hasErrors()) {
			//正ならnull,入力エラーの場合Field error in object 'form' on field 'email': rejected value [1 1];をstringで取得?
